<template>
  <div class="max-w-4xl mx-auto py-8 px-4 bg-[#0f172a] min-h-screen text-slate-200 rounded-xl">
      ...
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="text-right">
          ...
          <p class="text-xl font-black text-blue-400">
            {{ correctCount }}/{{ shuffledExercises.length }}
          </p>
        </div>
      </div>
    ...
    <div
      v-if="lives <= 0"
      class="text-center py-20 bg-[#1e293b] rounded-3xl border-2 border-red-900/50 animate-in fade-in zoom-in"
    >
    ...
    <div
      v-else-if="correctCount === shuffledExercises.length && shuffledExercises.length > 0"
      class="text-center py-20 bg-[#1e293b] rounded-3xl border-2 border-green-900/50"
    >
    ...
    <div v-else class="space-y-6 mt-6">
      ...
      >
        ...
        <div
          class="text-xl font-medium leading-relaxed flex flex-wrap items-center gap-x-2 gap-y-4"
        >
        ...
        <div class="mt-6 flex flex-col gap-3">
          <div class="flex items-center gap-4">
            ...
            >
              <UIcon
                :name="showHint[ex.id] ? 'i-heroicons-light-bulb-solid' : 'i-heroicons-light-bulb'"
                class="w-4 h-4"
              />
          ...
          >
            <div
              v-if="showHint[ex.id]"
              class="bg-slate-900/50 border border-slate-700 p-4 rounded-xl"
            >
...
const checkAnswer = (ex) => {
  ...
  if (userAns === ex.hiddenWord.toLowerCase()) {
    ...
    lessonStore.markAsMastered(ex.hiddenWord)
